...
| Missing `server.on('error')` on `app.listen` → EADDRINUSE crash-loop (`index.ts:9`). | red-herring | **Reproduced live on this host:** express 5.2.1 *swallows* EADDRINUSE with no error listener — no uncaught throw, no crash-loop. Latent Linux/raw-net portability gap, not a cause. |
